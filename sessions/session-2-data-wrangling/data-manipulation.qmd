...
library(janitor) # Used for cleaning data.frame names
...
As there are a different number of observations per cereal type, it maybe good to record this too. For a full column like wheat, the `n()` does this job for us. However for oats, we want to count all the times that oats is not `NA`.
